...
from lvdm.models.modules.attention_temporal import MemoryEfficientCrossAttention, STAttentionBlock, CrossAttention, TemporalCrossAttention, FeedForward
...
class Perceiver_layer(nn.Module):
    ...
    def _forward(self, text, video):
        ...
        video = self.attn1(self.norm1(video)) + video
        video = self.ff1(self.norm2(video)) + video
        video = rearrange(video, '(b t) l c -> (b l) t c', b=b, t=t, l=l)
        video = self.attn1_tmp(self.norm3(video)) + video
        video = self.ff2(self.norm4(video)) + video
        video = rearrange(video, '(b l) t c -> (b t) l c', b=b, t=t, l=l)
        video = self.attn2(self.norm5(video), context=text) + video
        video = self.ff3(self.norm6(video)) + video
        video = rearrange(video, '(b t) l c -> (b l) t c', b=b, t=t, l=l)
        text = repeat(text, 'b t c -> b l t c', l=64)
        text = rearrange(text, 'b l t c -> (b l) t c')
        video = self.attn2_tmp(self.norm7(video)) + video
        video = self.ff4(self.norm8(video)) + video
...
class Action_Prism(nn.Module):
    def __init__(self, 
        ...
        self.latent_to_T = nn.Sequential(
            ...
            nn.Linear(in_features=latent_query_dim * 4, out_features=latent_query_dim * 4),
            nn.Linear(in_features=latent_query_dim * 4, out_features=4))
        self.latent_to_S = nn.Sequential(
            nn.Linear(in_features=latent_query_dim, out_features=latent_query_dim * 4),
            nn.Linear(in_features=latent_query_dim * 4, out_features=latent_query_dim * 4),
            nn.Linear(in_features=latent_query_dim * 4, out_features=4))
    ...
    def forward(self, text, video):
        ...
        text = rearrange(text, 'b t l d -> (b t) l d')
        # video = rearrange(video, 'b t l c -> b t l c')
        for l in self.net:
            video = l(text, video)
        
        Tout = rearrange(video, 'b t l c -> (b l) t c')
        Tout = self.latent_to_T(Tout)
        Tout = rearrange(Tout, '(b l) t c -> b t (l c)', b=b)
        
        Sout = rearrange(video, 'b t l c -> (b t) l c')
        Sout = self.latent_to_S(Sout)
        Sout = rearrange(Sout, '(b t) l c -> b l (t c)', b=b)
        return [Sout, Tout]
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.out = nn.Sequential(
            normalization(ch),
            nonlinearity(nonlinearity_type),
            conv_nd(dims, model_channels, out_channels, (kernel_size_t, 3,3), padding=(padding_t, 1,1)),
        )
    ...
    def forward(self, x, timesteps=None, time_emb_replace=None, context=None, y=None, inject_video=None, inject_text=None, **kwargs):
        ...
        if time_emb_replace is None:
            ...
            t_emb = t_emb.to(next(self.time_embed.parameters()).data.dtype)
        ...
        if self.EchoReel == True:
            ...
            kwargs['Tout'] = Tout
        # h = x.type(self.dtype)
        h = x.type(next(self.time_embed.parameters()).data.dtype)
