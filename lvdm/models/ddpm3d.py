...
def save_gif(tensor_images, path):
        images = tensor_images.mul(255).add_(0.5).clamp_(0, 255).permute(0, 2, 3, 1).to("cpu", torch.uint8).numpy()
        # images = images.type(torch.uint8).cpu()
        # images = images.numpy()
        # images = images.transpose((0, 2, 3, 1))
        imageio.mimsave(path, images, format='GIF', fps=5, loop=0)
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.name = name
        self.current_train_batch_idx=0
    ...
    def validation_step(self, batch, batch_idx):
        img_dict = self.log_images(batch, len(batch['input_text']), unconditional_guidance_scale=15)
        for i in range(len(batch['input_text'])):
            text = batch['input_text'][i]
            video = img_dict['samples'][i].float()
            ...
            try:
                save_image(videos / 2 + 0.5, os.path.join('videos', self.name, str(self.current_epoch) + '_' + str(self.current_train_batch_idx) + '_' + str(self.rank + i * self.world_size) + '_' + text + '.jpg'))
                save_gif(video1 / 2 + 0.5, os.path.join('videos', self.name, str(self.current_epoch) + '_' + str(self.current_train_batch_idx) + '_' + str(self.rank + i * self.world_size) + '_sample_' + text + '.gif'))
                save_gif(video / 2 + 0.5, os.path.join('videos', self.name, str(self.current_epoch) + '_' + str(self.current_train_batch_idx) + '_' + str(self.rank + i * self.world_size) + '_ref_' + text + '.gif'))
            except:
                pass
    ...
    def test_step(self, batch, batch_idx):
        ...
        for i in range(len(batch['input_text'])):
            ...
            videos = torch.cat((video, video1), dim=0)
            try:
                save_image(videos / 2 + 0.5, os.path.join('videos', self.name, '_' + str(batch_idx) + '_' + str(self.rank + i * self.world_size) + '_' + text + '.jpg'))
                save_gif(video1 / 2 + 0.5, os.path.join('videos', self.name, '_' + str(batch_idx) + '_' + str(self.rank + i * self.world_size) + '_sample_' + text + '.gif'))
                save_gif(video / 2 + 0.5, os.path.join('videos', self.name, '_' + str(batch_idx) + '_' + str(self.rank + i * self.world_size) + '_ref_' + text + '.gif'))
            except:
                pass
    
    def video_create(self, batch):
        img_dict = self.log_images(batch, len(batch['input_text']), unconditional_guidance_scale=12)
        # i = 0
        # text = batch['input_text'][i]
        # video = img_dict['samples'][i].float()
        # video = video.permute(1, 0, 2, 3).cpu()
    ...
    def encode_video_frames(self, x, c):
        b, _, t, h, w = x.shape
    ...
    @torch.no_grad()
    def get_input(self, batch, k, return_text=False,
        ...
        #     xrec = self.decode_first_stage(z1)
    ...
    def shared_step(self, batch, **kwargs):
        ...
        """
        x1, c1, x2, c2 = self.get_input(batch, self.first_stage_key)
        loss = self(x1, c1, x2, c2)
        return loss
    
    def forward(self, x1, c1, x2, c2, *args, **kwargs):
    ...
    def p_losses(self, x_start, cond, inject_video, inject_text, t, noise=None, skip_qsample=False, x_noisy=None, cond_mask=None, **kwargs,):
        ...
        self.lvlb_weights = self.lvlb_weights.to(loss_vlb.device)
        loss_vlb = (self.lvlb_weights[t.long()] * loss_vlb).mean()
    ...
    def sample_log(self, cond, batch_size, ddim, ddim_steps, shape=None, **kwargs):
        if ddim:
            ...
            if shape is None:
                shape = (self.channels, 16, *self.image_size)
    ...
    @torch.no_grad()
    def log_images(self, batch, N=1, n_row=4, sample=True, ddim_steps=50, ddim_eta=1., unconditional_guidance_scale=1.0, 
        ...
        # get input
        z1, c1, z2, c2, uncond_embd = self.get_input(batch, 
                                           k=self.first_stage_key,
                                           return_text=True,
                                           cond_key=None,
                                           )
        kwargs["unconditional_conditioning"] = uncond_embd
