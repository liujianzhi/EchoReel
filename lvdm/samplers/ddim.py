...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
        ...
        b = shape[0]
        if x_T is None:
            img = torch.randn(shape, device=device).to(self.model.dtype)
            # img = torch.tensor(np.load('/home/liujianzhi/data/LVDM/noise.npy')).to(self.model.dtype).cuda()
        else:
            img = x_T
        ...
        for i, step in enumerate(iterator):
            ...
            # ts = torch.full((b,), step, device=device, dtype=self.model.dtype)
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if is_video:
            ...
            size = (b, 1, 1, 1)
        a_t = torch.full(size, alphas[index], device=device).to(e_t.dtype)
        a_prev = torch.full(size, alphas_prev[index], device=device).to(e_t.dtype)
        sigma_t = torch.full(size, sigmas[index], device=device).to(e_t.dtype)
        sqrt_one_minus_at = torch.full(size, sqrt_one_minus_alphas[index],device=device).to(e_t.dtype)
        ...
        if sample_noise is None:
            noise = sigma_t * noise_like(x.shape, device, repeat_noise).to(e_t.dtype) * temperature
            if noise_dropout > 0.:
